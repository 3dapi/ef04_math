...
INT CMcGrid::Create(LPDIRECT3DDEVICE9 pDev)
{
	...
	for(i=0; i<20; ++i)
	{
		pLine[8*i +0 ] = VtxD(-200, 0,  10.0F* (i+1), (i%2)? d1 : d2);
		pLine[8*i +1 ] = VtxD( 200, 0,  10.0F* (i+1), (i%2)? d1 : d2);
		pLine[8*i +2 ] = VtxD(-200, 0, -10.0F* (i+1), (i%2)? d1 : d2);
		pLine[8*i +3 ] = VtxD( 200, 0, -10.0F* (i+1), (i%2)? d1 : d2);

		pLine[8*i +4 ] = VtxD(  10.0F* (i+1), 0,-200, (i%2)? d1 : d2);
		pLine[8*i +5 ] = VtxD(  10.0F* (i+1), 0, 200, (i%2)? d1 : d2);
		pLine[8*i +6 ] = VtxD( -10.0F* (i+1), 0,-200, (i%2)? d1 : d2);
		pLine[8*i +7 ] = VtxD( -10.0F* (i+1), 0, 200, (i%2)? d1 : d2);
	}
