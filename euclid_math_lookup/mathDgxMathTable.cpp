...
#define DGX_EPSILON		0.00001
#define DGX_ALMOST_ONE	0.99999
...
void DGX_MakeCosSinTable()
{
	...
	for(i=0; i<361; ++i)
	{
		Rad = i * DGX_PI /180.;
		Cos = (float)cos(Rad);
		if(-DGX_EPSILON <= Cos && Cos <= DGX_EPSILON)
			Cos = 0;
		else if(-DGX_ALMOST_ONE >= Cos)
			Cos = -1;
		else if(DGX_ALMOST_ONE <= Cos)
			Cos = 1;
	...
	for(i=0; i<361; ++i)
	{
		...
		if(-DGX_EPSILON <= Sin && Sin <= DGX_EPSILON)
			Sin = 0;

		else if(-DGX_ALMOST_ONE >= Sin)
			Sin = -1;
